...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    static class Node<K, V> implements Map.Entry<K, V> {
        final int hash;
        final K key;
        volatile V val;// 加了volatile保证缓存一致性
    ...
    static final int spread(int h) {
        // 前段部分和HashMap的hashCode算法相同，但后段部分有点像HashTable
        /**
         * {@link HashMap#hash(Object)}
         * {@link Hashtable#put(Object, Object)}
         */
    ...
    static final <K, V> Node<K, V> tabAt(Node<K, V>[] tab, int i) {
         ...
         *
         * 作者：占小狼
         * 链接：https://www.jianshu.com/p/c0642afe03e0
         * 来源：简书
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            // 当sizeCtl小于0时，说明另外的线程在执行初始化，让出时间片让另外的线程执行
            if ((sc = sizeCtl) < 0)
                Thread.yield(); // lost initialization race; just spin
            else if (U.compareAndSetInt(this, SIZECTL, sc, -1)) {// CAS比较sc的值是否和偏移的相等，相等就设置为-1。-1说明在初始化
                try {
                    // 双重判断？有点熟悉
                    if ((tab = table) == null || tab.length == 0) {
                        int n = (sc > 0) ? sc : DEFAULT_CAPACITY;
                        @SuppressWarnings("unchecked")
                        Node<K, V>[] nt = (Node<K, V>[]) new Node<?, ?>[n];// 新建Node数组
                        table = tab = nt;
                        sc = n - (n >>> 2);// 假如n = 16, 16 - (16 >>> 2) = 12，相当于16 * 0.75 = 12
                    }
                } finally {
                    sizeCtl = sc;
                }
